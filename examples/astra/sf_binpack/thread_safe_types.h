...
class ThreadSafeRingBuffer {
  ...
  public:
    ThreadSafeRingBuffer() { m_ringBuffer.resize(Capacity); }
...
class ThreadSafeRingBuffer<T, 1> {
  public:
    ThreadSafeRingBuffer() { m_slot.resize(1); }
    ...
    bool put(T& incomingItem, Pred stopCondition) {
        ...
        m_ringNotFull.wait(lock, [this, &stopCondition]() { return !m_isFull || stopCondition(); });

        if (stopCondition() && m_isFull)
            return false;
    ...
    bool take(T& outgoingItem, Pred stopCondition) {
        ...
        m_ringNotEmpty.wait(lock, [this, &stopCondition]() { return m_isFull || stopCondition(); });
    ...
    void reserve_internal(size_t size) { m_slot.reserve(size); }
...
struct ThreadLocalIndex {
    ...
    ThreadLocalIndex()
        : id(ThreadIdManager::acquire()) {}

    ~ThreadLocalIndex() { ThreadIdManager::release(id); }
  ...
  public:
    ...
    T& get(uint32_t thread_idx = ThreadLocalIndex::get()) {
        ...
        if (m_chunks[chunk_idx].compare_exchange_strong(
                expected, new_chunk, std::memory_order_release, std::memory_order_acquire
            )) {
