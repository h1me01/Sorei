...
class GraphBuilder {
  public:
    ...
    ParamNode param(int input_dim, int output_dim) {
        return {this, graph_.emplace<Param>(matrix::Shape{output_dim, input_dim})};
    }
    ...
    Node relu(const Node& x) { return node<ElemwiseUnary>(x.get(), unary::ReLU{}); }

    // relu clamped to [0, 1]
    Node clamped_relu(const Node& x) { return node<ElemwiseUnary>(x.get(), unary::ClampedReLU{}); }

    // relu clamped to [0, 1] and squared
    Node squared_clamped_relu(const Node& x) {
        return node<ElemwiseUnary>(x.get(), unary::SquaredClampedReLU{});
    }

    Node sigmoid(const Node& x) { return node<ElemwiseUnary>(x.get(), unary::Sigmoid{}); }

    Node abs(const Node& x) { return node<ElemwiseUnary>(x.get(), unary::Abs{}); }

    Node clamp(const Node& x, float lo, float hi) {
        return node<ElemwiseUnary>(x.get(), unary::Clamp{lo, hi});
    }
    ...
    Node div(const Node& x, float s) { return affine_unary(x, 1.0f / s, 0.0f); }
    Node div(float s, const Node& x) { return node<ElemwiseUnary>(x.get(), unary::DivLeft{s}); }

    Node add(const Node& a, const Node& b) {
        return node<ElemwiseBinary>(a.get(), b.get(), binary::Add{});
    }

    Node sub(const Node& a, const Node& b) {
        return node<ElemwiseBinary>(a.get(), b.get(), binary::Sub{});
    }

    Node mul(const Node& a, const Node& b) {
        return node<ElemwiseBinary>(a.get(), b.get(), binary::Mul{});
    }

    Node div(const Node& a, const Node& b) {
        return node<ElemwiseBinary>(a.get(), b.get(), binary::Div{});
    }

    Node mat_mul(const Node& weight, const Node& input) {
        return node<MatMul>(weight.get(), input.get());
    }

    Node affine(const Node& input, const Node& weight, const Node& bias) {
        auto* ii = dynamic_cast<InputInt*>(input.get());
        if (ii)
            return node<SparseAffine>(ii, weight.get(), bias.get());
        else
            return node<Affine>(input.get(), weight.get(), bias.get());
    }

    Node select(const Node& input, BucketIndex* index) { return node<Select>(input.get(), index); }

    Node pairwise_mul(const Node& input) { return node<PairwiseMul>(input.get()); }

    Node concat(std::vector<Node> inputs, ConcatAxis axis = ConcatAxis::Rows) {
        std::vector<Layer*> layers;
        for (const auto& n : inputs)
            layers.push_back(n.get());
        return node<Concat>(std::move(layers), axis);
    }

    Node mean(const Node& input) { return node<Mean>(input.get()); }
    ...
    Node softmax_cross_entropy(const Node& logits, const Node& labels) {
        ...
        return node<SoftmaxCrossEntropy>(logits.get(), li);
    }
  ...
  private:
    ...
    Node node(Args&&... args) {
        return {this, graph_.emplace<T>(std::forward<Args>(args)...)};
    }

    Node affine_unary(const Node& x, float scale, float bias) {
        return node<ElemwiseUnary>(x.get(), unary::AddScale{scale, bias});
    }
};
